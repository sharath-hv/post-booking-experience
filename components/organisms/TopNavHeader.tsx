...
export type TopNavHeaderProps = {
  ...
  /**
   * Opaque header fill (no fade-out gradient). Use on standalone picker/form
   * pages; leave off for concierge/aurora screens. Loan application uses
   * `inverted` instead and does not show this backdrop.
   */
  solid?: boolean;
  className?: string;
};
  ...
  return (
    <header
      className={cn(
        ...
        solid && !inverted
          ? surface === "white"
            ? styles.bg_solid_white
            : styles.bg_solid_default
          : null,
        className
      )}
    >
