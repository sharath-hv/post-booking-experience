 ...
 * `/booking/received`; modify-selection returns use the auto-advance
 * payment-received ack at `/booking/confirmed` (next route journey-aware).
 */
export function buildBookingLockSuccessHref(
  ...
  if (!options?.returnSource) {
    return `/booking/received?${q.toString()}`;
  }
  q.set("source", "payment");
  q.set("return_source", options.returnSource);
  return `/booking/confirmed?${q.toString()}`;
}
...
export function buildCarDeliveryInsurancePrepHref(params?: InsuranceJourneyQuery): string {
  ...
  if (params?.addons) q.set("addons", params.addons);
  const qs = q.toString();
  return qs ? `/delivery/insurance-prep?${qs}` : "/delivery/insurance-prep";
}
