...
import type { ExperienceFlow } from "@/helpers/experience-flow";
import { normalizeAppPathname } from "@/helpers/journey-routes";
...
export function isVehicleRegistrationAvailable(pathname: string): boolean {
  ...
  return (
    ...
    path.includes("car-delivery-schedule")
  );
}
...
export function resolveJourneyStageIndex(pathname: string): number {
  ...
  if (path === "/booking/received" || path === "/payment/booking-success") return 0;
  if (path.startsWith("/delivery") || path.startsWith("/payment/car-delivery")) return 3;
  if (path.startsWith("/payment")) return 2;
  // Car reserved (VIN on card) — exact-car chapter is done; money is next.
  if (path === "/booking/confirmed" || path === "/car-allocation/confirmed") {
    return 2;
  }
  if (
...
const STEP_COPY: readonly StepCopy[] = [
  ...
  },
  {
    ...
    done: "Collected. Enjoy the drive.",
    now: "Insurance, RTO, then pickup at the dealership",
 ...
 * chapter headline. Delivery chapter is "Collect your car" but often the ask is
