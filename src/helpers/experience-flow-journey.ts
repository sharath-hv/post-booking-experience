...
import { MODIFY_SELECTION_RETURN_SOURCE } from "@/helpers/paymentUrls";
...
function isModifySelectionBookingReceivedPath(
  ...
  if (path !== JOURNEY_PATHS.booking.confirmed) return false;
  return (
...
function isModifySelectionPath(path: string): boolean {
  return (
    path === JOURNEY_PATHS.booking.modify ||
    path.startsWith(`${JOURNEY_PATHS.booking.modify}/`) ||
    path === "/kyc/modify-selection" ||
    path.startsWith("/kyc/modify-selection/")
  );
}

function isCancelBookingPath(path: string): boolean {
  return (
    path === JOURNEY_PATHS.booking.cancel ||
    path.startsWith(`${JOURNEY_PATHS.booking.cancel}/`) ||
    path === "/kyc/cancel-booking" ||
    path.startsWith("/kyc/cancel-booking/")
  );
}
...
export function isModifyNoChargesFlowPathAllowed(
  ...
  if (path === JOURNEY_PATHS.identity.hub || isModifySelectionPath(path)) {
    return true;
  }
...
export function getModifyNoChargesRedirectTarget(
  ...
  if (
    ...
    return JOURNEY_PATHS.identity.hub;
  }
 ...
 * to `/booking/accepted` (partner locked — fee boundary). Returns `null` when no redirect is needed.
 */
export function getCancelWithChargesRedirectTarget(
  ...
  if (isModifySelectionPath(path)) {
    return JOURNEY_PATHS.booking.accepted;
  }
