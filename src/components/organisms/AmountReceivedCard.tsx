...
export type AmountReceivedCardProps = {
  ...
  note?: string;
  /**
   * `received` — paid / settled (green tick).
   * `processing` — in flight (amber spinner).
   * `due` — still to pay (grey money glyph; not a success / loading state).
   */
  status?: "received" | "processing" | "due";
  /** Custom icon image to replace the default tick / spinner / due glyph. */
  iconSrc?: string | StaticImageData;
  /** Icon well tone (overrides the green / amber / grey default). */
...
export function AmountReceivedCard({
  ...
  // Custom icons + due use grey; processing is amber; received is green.
  const tone: IconWellTone =
    iconTone ?? (iconSrc || due ? "grey" : processing ? "amber" : "green");
  return (
    <div
      className={cn(
