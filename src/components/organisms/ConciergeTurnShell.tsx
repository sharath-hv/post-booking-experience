...
import {
  ...
  ZOOM_OVERLAY_MS,
} from "@/components/organisms/ManageBookingZoomOverlay";
import { ShiviCallSheet } from "@/components/organisms/ShiviCallSheet";
import { ShiviDialogue } from "@/components/organisms/ShiviDialogue";
import { TimeSkipChip } from "@/components/molecules/TimeSkipChip";
import { UserEchoChip } from "@/components/molecules/UserEchoChip";
import {
  WorkingNarration,
...
import { getDeliveryDateShort } from "@/helpers/journey-stage";
...
export type ConciergeTurnShellProps = ConciergeTurn & {
  ...
  /**
   * Consume any pending handoff chip without showing it — news turns after
   * payment/celebration where the user's last CTA isn't conversational.
   */
...
export function ConciergeTurnShell({
  ...
  useEffect(() => {
    ...
    const echoText = suppressEcho ? null : pending;
    setEcho(echoText);
    if (echoText == null || instantRevealEnabled()) {
