...
import { type BuyingGuideStep } from "@/constants/buying-guide-content";
...
type BuyingGuideSectionProps = {
  step: BuyingGuideStep;
};

/** Hero image + step copy. */
export function BuyingGuideContent({ step }: BuyingGuideSectionProps) {
  return (
    <div className={cn(styles.mx_auto_0, MAIN_BOTTOM_PADDING_CLASS)}>
      <RevealStagger
        key={`image-${step.step}`}
      ...
      <div className={styles.mt_8_2} key={`copy-${step.step}`}>
        ...
        <RevealStagger delay={STAGGER_TITLE}>
          <h1 className={styles.text_20px__4}>{step.title}</h1>
        </RevealStagger>
...
/** Fixed onboarding CTA. */
export function BuyingGuideCtaFooter({ step }: BuyingGuideSectionProps) {
...
export function BuyingGuideScreen({ step }: BuyingGuideSectionProps) {
  return (
    <>
