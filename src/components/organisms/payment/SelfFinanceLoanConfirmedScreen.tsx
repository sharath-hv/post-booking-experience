...
import { buildMarginMoneySlipActionHref } from "@/helpers/paymentUrls";
import { PARTNER_DEALER_LABEL, PARTNER_DEALER_LABEL_CAPITALIZED } from "@/constants/dealer-attribution-content";
...
export function SelfFinanceLoanConfirmedScreen() {
  ...
  const downPaymentInr = useMemo(
    () => cashDownPaymentDueInr(loanAmountInr),
    [loanAmountInr],
  );
  ...
  const downPaymentPaidHref = buildMarginMoneySlipActionHref({
    ...
    originalDownPaymentInr: downPaymentInr,
  });
