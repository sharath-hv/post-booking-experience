 ...
 * Pay → loader → dedicated success route (`/booking/confirmed?source=payment` or `/payment/down-payment-success`).
...
export function PaymentCheckoutScreen({ header }: { header: ReactNode }) {
  ...
  const checkoutMeta = useMemo(
    () => readPaymentCheckoutQuery(searchParams),
    [searchParams],
  );
      ...
      <div className={[styles.fixed_32, "footer-elevated"].filter(Boolean).join(" ")}>
        <div className={styles.mx_auto_33}>
          <PrimaryCta
            ...
            className={styles.primary_cta_34}
          >
            {isInsurancePayment
