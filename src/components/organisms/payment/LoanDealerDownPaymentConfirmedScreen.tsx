...
/** Persist confirmed beat in the URL so manage-booking summary can show DP as paid. */
export const DP_CONFIRMED_QUERY = "dp_confirmed";
...
export function LoanDealerDownPaymentConfirmedScreen() {
  ...
  const dealerConfirmed = searchParams.get(DP_CONFIRMED_QUERY) === "1";

  const downPaymentInr = useMemo(() => cashDownPaymentDueInr(loanAmountInr), [loanAmountInr]);

  const dealerConfirmedHref = useCallback(
    (confirmed: boolean) => {
      const params = new URLSearchParams();
      if (bankId) params.set("bank", bankId);
      params.set("loan_amount", String(loanAmountInr));
      if (confirmed) params.set(DP_CONFIRMED_QUERY, "1");
      const qs = params.toString();
      return qs
        ? `/payment/down-payment-dealer-confirmed?${qs}`
  ...
  const onBack = useCallback(() => {
    ...
    if (dealerConfirmed) {
      router.replace(dealerConfirmedHref(false));
      return;
    }
    const params = new URLSearchParams();
    if (bankId) params.set("bank", bankId);
  ...
  }, [bankId, dealerConfirmed, dealerConfirmedHref, router]);

  const markDealerConfirmed = useCallback(() => {
    router.replace(dealerConfirmedHref(true));
  }, [dealerConfirmedHref, router]);
