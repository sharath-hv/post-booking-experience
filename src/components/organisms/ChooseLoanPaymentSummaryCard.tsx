...
export type ChooseLoanPaymentSummaryCardProps = {
  ...
  /** Full or remaining down payment. */
  downPaymentAmountInr: number;
  /** When set with full pay, shows “Down payment paid”; otherwise “to be paid”. */
  downPaymentPaidInr?: number;
  /** True when the full car down payment is settled. */
...
/**
 * Manage-booking loan payment summary.
 * Order: ACKO price → booking → down payment → loan.
 */
export function ChooseLoanPaymentSummaryCard({
  ...
  const showDownPaymentPaidRow =
    downPaymentFullyPaid &&
    downPaymentPaidInr != null &&
    downPaymentPaidInr > 0;
  const showPartialRemainingFooter =
    !downPaymentFullyPaid &&
    downPaymentPaidInr != null &&
    downPaymentPaidInr > 0 &&
    downPaymentAmountInr > 0;

  const downPaymentDisplayInr = showDownPaymentPaidRow
    ? downPaymentPaidInr!
    : showPartialRemainingFooter
      ? downPaymentPaidInr! + downPaymentAmountInr
      : downPaymentAmountInr;
  ...
  return (
    ...
    >
      <div className={cn(styles.px_4_13, !isGlass && styles.bg_white_14)}>
        ...
        <div className={styles.flex_4}>
          <span className={styles.text_sm_2}>
            {showDownPaymentPaidRow
              ? "Down payment paid"
              : "Down payment to be paid"}
          </span>
          <span className={styles.text_sm_10}>
            {formatInr(downPaymentDisplayInr)}
          </span>
        </div>
      ...
      {showPartialRemainingFooter ? (
        ...
        >
          <span className={styles.text_base_11}>Remaining down payment</span>
