...
| `/payment/loan-rejected` | Bank declined — analysis outcomes (`?outcome=` demo switcher); radio option cards + contextual CTA (`LoanRejectedScreen`, `src/helpers/loan-rejected-content.ts`). Cancel outs → `/booking/cancel?stage=post&from=finance` (50% fee; echo **Cancel my booking**) |
...
- `src/components/organisms/payment/LoanRejectedScreen.tsx`, `LoanRejectedOutcomeSwitcher.tsx`, `src/helpers/loan-rejected-content.ts`
